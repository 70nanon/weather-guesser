...
- **場所モード**: 今日の天気だけを手がかりに、場所を当てる（日本が標準。世界は高難易度）
...
ゲーム開始時に、選んだ範囲（日本 / 世界）の都市リストからランダムに「正解地点」が選ばれます。地点名は隠し、その現在の天気だけを見せます。
プレイヤーは最大 3 回まで任意の地点の天気を調査し、天気の差から場所を絞り込み、地図上で回答します。

- デフォルトは日本（都道府県庁所在地）。世界は高難易度
- 途中で範囲を切り替えると、進行中の問題は捨てて新しいラウンドになる
...
src/
  ...
  data/         targetCities.ts / japanCities.ts  場所モードの正解候補
